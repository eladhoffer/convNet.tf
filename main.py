    ...
    for var in var_list:
        if var is not None:
            ...
            if len(sz) == 4 and sz[2] == 3:
                ...
                tf.image_summary(var.op.name + '/kernels',
                                 group_batch_images(kernels), max_images=1)
    for activation in activation_list:
        if activation is not None:
            tf.histogram_summary(activation.op.name +
                                 '/activations', activation)
            #tf.scalar_summary(activation.op.name + '/sparsity', tf.nn.zero_fraction(activation))
...
def train(model, data,
    ...
    with tf.device('/cpu:0'):
        ...
        tf.image_summary('training_images', group_batch_images(x))
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.name_scope('objective'):
            loss = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(y, yt))
        ...
        opt = tf.contrib.layers.optimize_loss(loss, global_step, learning_rate, 'Adam',
                                              ...
                                              learning_rate_decay_fn=None, update_ops=None, variables=None, name=None)
    ...
    tf.scalar_summary('loss/training', loss_avg)
    accuracy_avg = ema.average(accuracy)
    tf.scalar_summary('accuracy/training', accuracy_avg)
